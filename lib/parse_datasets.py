...
from latent_ode.heparin import HeparinDataset, variable_time_collate_fn, get_data_min_max
from latent_ode.physionet import PhysioNet, variable_time_collate_fn, get_data_min_max
from latent_ode.person_activity import PersonActivity, variable_time_collate_fn_activity
...
def parse_datasets(args, device):
    ...
    if dataset_name == "heparin":
        # The train dataset
        train_dataset_obj = HeparinDataset('data/heparin', 
                                           train=True,
                                           quantization=1.0,
                                           device=device,
        ...
        # The test dataset
        test_dataset_obj = HeparinDataset('data/heparin',
                                           train=False,
                                           quantization=1.0,
                                           device=device,
                                           build_from_file=True)

        # We want to extract the tensors from each of the objects
        # so that we can pass them into a pytorch neural network
        ...
        # We extract an example from train_data to get the right size
        # for the input_dims 
        record_id, tt, vals, mask, labels = train_data[0]
        ...
        train_dataloader = DataLoader(train_data, 
                                      ...
                                      shuffle=False, 
                                      collate_fn=lambda batch: variable_time_collate_fn(batch, 
                                                                                        args, 
                                                                                        device, 
                                                                                        data_type="train",
                                                                                        data_min=data_min,
        ...
        data_objects = {"dataset_obj": train_dataset_obj,
                        ...
                        "latents": 6,
                        "attr": attr_names,  # optional
                        "classif_per_tp": False,  # optional (only if you want to classify each time point)
                        "n_labels": 1}  # optional
